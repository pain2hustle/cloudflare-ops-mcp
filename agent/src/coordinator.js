...
import { HARNESS_VERSION, PERSONAL_ASSISTANT, TEMPLATES, suggestAgentName } from "./templates.js";
...
export class Coordinator extends Agent {
  ...
  async onStart() {
    ...
    this.sql`INSERT OR IGNORE INTO agent_versions (version,status,manifest_json,created_at) VALUES (${HARNESS_VERSION},${"active"},${JSON.stringify(manifest)},${Date.now()})`;
  ...
  async createJob(input, actor = "master") {
    ...
    this.sql`UPDATE agent_versions SET total_jobs=total_jobs+1 WHERE version=${HARNESS_VERSION}`;
    await this.log(id, "job_created", { packet_hash: packetHash, memory_hash: memoryHash, template_id: packet.template_id, template_title: packet.template_title, skill_ids: packet.skill_ids, agent_profile_id: profile.id, agent_name: packet.agent_name }, actor);
    ...
    try {
      return await this.keepAliveWhile(async () => {
        ...
        for (const proposal of [...(primary.result.proposed_revisions || []), ...(verifier?.result?.proposed_revisions || [])]) {
          ...
          this.sql`INSERT INTO revisions (id,job_id,status,proposal_json,created_at) VALUES (${revisionId},${id},${"proposed"},${JSON.stringify(candidate)},${Date.now()})`;
          await this.log(id, "revision_proposed", { revision_id: revisionId, title: candidate.title, target_skill: candidate.target_skill }, "worker");
        }
        const compact = {
          objective: packet.objective,
          template_id: packet.template_id,
          skill_ids: packet.skill_ids,
      ...
      if (!stopped) this.sql`UPDATE agent_versions SET failed_jobs=failed_jobs+1 WHERE version=${HARNESS_VERSION}`;
  ...
  async completeHealthJob(id, packet, sources) {
    ...
    const compact = { objective: packet.objective, template_id: packet.template_id, skill_ids: packet.skill_ids, agent_name: packet.agent_name, ai_calls: 0, checks };
    this.sql`UPDATE jobs SET status=${"completed"}, compact_json=${JSON.stringify(compact)}, updated_at=${now} WHERE id=${id}`;
    this.sql`UPDATE agent_versions SET completed_jobs=completed_jobs+1 WHERE version=${HARNESS_VERSION}`;
    const usage = await this.usageSummary();
